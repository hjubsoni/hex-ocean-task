...
function ModalForm({ handleDismiss }: { handleDismiss: () => void }) {
  ...
  const handleSubmit = async (
    ...
    e.preventDefault();
    let url = `https://umzzcc503l.execute-api.us-west-2.amazonaws.com/dishes?name=${order.name}&preparation_time=${order.preparation_time}&type=${order.type}`;
    if (order.type === 'pizza') {
      url = `${url}?no_of_slices=${Number(
        order.no_of_slices
      )}&diameter=${Number(order.diameter)}`;
    } else if (order.type === 'soup') {
      url = `${url}?spiciness_scale=${Number(order.spiciness_scale)}`;
    } else if (order.type === 'sandwich') {
      url = `${url}?slices_of_bread=${Number(order.slices_of_bread)}`;
    }
    try {
